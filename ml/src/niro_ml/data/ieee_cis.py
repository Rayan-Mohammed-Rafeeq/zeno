...
Column mapping to Niro canonical schema
───────────────────────────────────────
...
Niro canonical schema (they are payment-processor internal signals).
...
from niro_ml.data.schema import DataSource, RawTransaction
...
def _map_columns(tx_df: pd.DataFrame) -> pd.DataFrame:
    """
    Map IEEE-CIS columns to Niro canonical column names.
